...
Public surface used by ``Img2GPS/eval_project_a.py`` (the staff evaluator):
...
``load_raw(csv_path)`` is the same shape but un-normalized (values in [0, 1])
and is used by ``train.py`` so the gallery-building loop can apply
ImageNet normalization itself (matching DINOv2's input contract).

Both calls are cached on disk under ``<csv_dir>/.cache/<csv_stem>.<kind>.pt``
and re-used as long as the CSV file mtime has not advanced.
"""
...
def _read_csv_records(csv_path: str) -> Tuple[List[str], List[List[float]]]:
    ...
    image_col = _resolve_column(
        columns,
        ["image_path", "filepath", "image", "path", "file_name", "file_path", "filename"],
    )
...
def load_raw(csv_path: str) -> Tuple[torch.Tensor, torch.Tensor]:
    """(X_raw, y) where X_raw is [0, 1] float of shape (N, 3, 224, 224)."""
...
def prepare_data(csv_path: str) -> Tuple[torch.Tensor, torch.Tensor]:
    """ImageNet-normalized images and raw lat/lon labels (used by the
    backend evaluator). Output X is a (N, 3, 224, 224) float tensor."""
